   ...
   piezo_addr = fpga + PIEZO_OFFSET/sizeof(unsigned short);
	...
	while (1) {
	   ...
	   sw2 = ((*(unsigned int *) wheelButton_addr) >> 5)&0x1;
		if (sw1 == 1) {	//기존 휠 위 방향과 아래 방향에서 동일한 음이 소리가 나던 조건을 분리하여 각각 휠을 위로 돌렸을 때와 아래로 돌렸을 때 다른 소리가 나게 설정
	    printf("201602357 Sun Min Lee: SW1=%d\n", sw1, sw2);
			*piezo_addr = 0x05;
			usleep(500000);
			*piezo_addr = 0x06;
			usleep(500000);
      ...
      printf("201602357 Sun Min Lee: SW2=%d\n", sw1, sw2);
    	*piezo_addr = 0x12;
    	usleep(500000);
    	*piezo_addr = 0x13;
    	usleep(500000);
